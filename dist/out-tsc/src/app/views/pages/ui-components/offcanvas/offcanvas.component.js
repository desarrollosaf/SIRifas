...
const defaultOffcanvas = {
    htmlCode: `<ng-template #content let-offcanvas>
...
const offcanvasOptions = {
    htmlCode: `<ng-template #content2 let-offcanvas>
	<div class="offcanvas-header">
...
let OffcanvasComponent = class OffcanvasComponent {
    offcanvasService = inject(NgbOffcanvas);
    closeResult = '';
    defaultOffcanvasCode;
    offcanvasOptionsCode;
    ngOnInit() {
        this.defaultOffcanvasCode = defaultOffcanvas;
        this.offcanvasOptionsCode = offcanvasOptions;
    }
    openBasicOffcanvas(content) {
    ...
    openEndOffcanvas(content) {
        this.offcanvasService.open(content, { position: 'end' });
    }
    openTopOffcanvas(content) {
        this.offcanvasService.open(content, { position: 'top' });
    }
    openBottomOffcanvas(content) {
        this.offcanvasService.open(content, { position: 'bottom' });
    }
    openNoBackdropOffcanvas(content) {
        this.offcanvasService.open(content, { backdrop: false });
    }
    openStaticBackdropOffcanvas(content) {
        this.offcanvasService.open(content, { backdrop: 'static' });
    }
    openScrollOffcanvas(content) {
        this.offcanvasService.open(content, { scroll: true });
    }
    openNoKeyboardOffcanvas(content) {
        this.offcanvasService.open(content, { keyboard: false });
    }
    openNoAnimationOffcanvas(content) {
        this.offcanvasService.open(content, { animation: false });
    }
    openCustomBackdropClassOffcanvas(content) {
        this.offcanvasService.open(content, { backdropClass: 'bg-info' });
    }
    openCustomPanelClassOffcanvas(content) {
        this.offcanvasService.open(content, { panelClass: 'bg-info' });
    }
    scrollTo(element) {
...
//# sourceMappingURL=offcanvas.component.js.map