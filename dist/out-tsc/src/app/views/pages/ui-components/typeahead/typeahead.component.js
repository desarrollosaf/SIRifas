...
const defaultTypeahead = {
    htmlCode: `A typeahead example that gets values from a static <code>string[]</code>
<ul>
...
const resultTemplate = {
    htmlCode: `<ng-template #rt let-r="result" let-t="term">
...
let TypeaheadComponent = class TypeaheadComponent {
    model;
    resultTemplatemodel;
    defaultTypeaheadCode;
    resultTemplateCode;
    constructor() { }
    ngOnInit() {
        this.defaultTypeaheadCode = defaultTypeahead;
        this.resultTemplateCode = resultTemplate;
    }
    search = (text$) => text$.pipe(debounceTime(200), distinctUntilChanged(), map(term => term.length < 2 ? []
        : states.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10)));
    resultTemplateSearch = (text$) => text$.pipe(debounceTime(200), map(term => term === '' ? []
        : statesWithFlags.filter(v => v.name.toLowerCase().indexOf(term.toLowerCase()) > -1).slice(0, 10)));
    formatter = (x) => x.name;
    scrollTo(element) {
