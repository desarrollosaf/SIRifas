...
function ImageCropperComponent_div_3_ng_container_2_Template(rf, ctx) {
  if (rf & 1) {
    ...
    ɵɵlistener("mousedown", function ImageCropperComponent_div_3_ng_container_2_Template_span_mousedown_17_listener($event) {
      ɵɵrestoreView(_r5);
      const ctx_r1 = ɵɵnextContext(2);
    ...
    ɵɵlistener("mousedown", function ImageCropperComponent_div_3_ng_container_2_Template_span_mousedown_20_listener($event) {
      ɵɵrestoreView(_r5);
      const ctx_r1 = ɵɵnextContext(2);
...
var CropService = class {
  crop(cropperState, output) {
    ...
    const result = {
      ...
      height,
      imagePosition,
      cropperPosition: __spreadValues({}, cropperState.cropper)
    };
  ...
  cropToBlob(output, cropCanvas, cropperState) {
    return __async(this, null, function* () {
      output.blob = yield new Promise((resolve) => cropCanvas.toBlob(resolve, "image/" + cropperState.options.format, this.getQuality(cropperState.options)));
      if (output.blob) {
...
var LoadImageService = class {
  ...
  loadImageFile(file, cropperSettings) {
    return __async(this, null, function* () {
      ...
      return yield this.loadImageFromArrayBuffer(arrayBuffer, cropperSettings);
    });
  }
  ...
  loadImageFromArrayBuffer(arrayBuffer, cropperState, imageType) {
    return __async(this, null, function* () {
      ...
      return yield this.transformImageFromArrayBuffer(res, cropperState, res.originalImageSize != null);
    });
  }
  ...
  transformLoadedImage(loadedImage, cropperState, forceTransform = false) {
    return __async(this, null, function* () {
      ...
      const blob = yield new Promise((resolve) => canvas.toBlob(resolve, cropperState.options.format));
      if (!blob) {
...
var ImageCropperComponent = class _ImageCropperComponent {
  ...
  reset() {
    this.safeImgDataUrl.set("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAC0lEQVQYV2NgAAIAAAUAAarVyFEAAAAASUVORK5CYII=");
    this.state.loadedImage = void 0;
    this.state.maxSize = void 0;
    this.imageVisible = false;
  }
  loadImageFile(file) {
  ...
  checkImageMaxSizeRecursively() {
    if (this.setImageMaxSizeRetries > 40) {
      ...
      this.cropperReady.emit(__spreadValues({}, this.state.maxSize));
      this.doAutoCrop();
  ...
  onResize() {
    ...
    if (this.hidden) {
      ...
      const oldMaxSize = __spreadValues({}, this.state.maxSize);
      this.setMaxSize();
  ...
  cropToBlob() {
    return new Promise((resolve, reject) => this.zone.run(() => __async(this, null, function* () {
      const result = yield this.cropService.crop(this.state, "blob");
      if (result) {
  ...
  static {
    this.ɵfac = function ImageCropperComponent_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _ImageCropperComponent)(ɵɵdirectiveInject(DomSanitizer), ɵɵdirectiveInject(ChangeDetectorRef), ɵɵdirectiveInject(NgZone));
    };
  }
  static {
    this.ɵcmp = ɵɵdefineComponent({
      ...
      template: function ImageCropperComponent_Template(rf, ctx) {
        ...
        if (rf & 2) {
          ...
          ɵɵadvance();
          ɵɵstyleProp("width", (ctx.state.maxSize == null ? null : ctx.state.maxSize.width) || 0, "px")("height", (ctx.state.maxSize == null ? null : ctx.state.maxSize.height) || 0, "px")("margin-left", ctx.alignImage === "center" ? ctx.marginLeft : null);
          ɵɵadvance();
